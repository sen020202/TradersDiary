...
def get_nearest_expiry(df, symbol):
    ...
    if symbol_df.empty:
        print(f"?? No NFO instruments found for {symbol}")
        return None
    ...
    if not exp_dates:
        print(f"?? Could not parse any expiry dates for {symbol}")
    ...
    print(f"?? All expiries in past, using: {exp_dates[-1][1]}")
...
def analyze_symbol(api, df, symbol):
    ...
    if not expiry:
        print(f"?? Cannot proceed without expiry")
        return pd.DataFrame()
    ...
    for i, strike in enumerate(strikes, 1):
        ...
        # ---- CE historical data ----
        print("  Fetching CE last candle (previous day)...")
        df_ce_last = get_historical_option_data(api, df, symbol, expiry, strike, "CE", last_start, last_end, INTERVAL)
    
        print("  Fetching CE first candle (today)...")
        df_ce_first = get_historical_option_data(api, df, symbol, expiry, strike, "CE", first_start, first_end, INTERVAL)
    
        # ---- PE historical data ----
        print("  Fetching PE last candle (previous day)...")
        df_pe_last = get_historical_option_data(api, df, symbol, expiry, strike, "PE", last_start, last_end, INTERVAL)
    
        print("  Fetching PE first candle (today)...")
        df_pe_first = get_historical_option_data(api, df, symbol, expiry, strike, "PE", first_start, first_end, INTERVAL)
    
        # -------------------------
        #  CE Previous Day OHL
        # -------------------------
        PDO = float(df_ce_last.iloc[0]['open']) if df_ce_last is not None and not df_ce_last.empty else None
        PDH = float(df_ce_last.iloc[0]['high']) if df_ce_last is not None and not df_ce_last.empty else None
        PDL = float(df_ce_last.iloc[0]['low'])  if df_ce_last is not None and not df_ce_last.empty else None
    
        # CE Last Price (previous day close)
        ...
        # Today's first candle highs/lows
        ...
        # -------------------------
        # Append results with PDO/PDH/PDL added
        # -------------------------
        results.append({
            "Strike": strike,
    
            # NEW COLUMNS beside CE_LTP
            "PDO": PDO,
            "PDH": PDH,
            "PDL": PDL,
            ...
            "PE_LTP": PE_LTP,
            "AvgPremium": avg_prem,
            "SPOT-STRIKE_CE": spot_open - strike if spot_open else None,
            "STRIKE-SPOT_PE": strike - spot_open if spot_open else None,
            "UDS_CE": uds_ce,
            "UDS_PE": uds_pe
        })
...
def main():
    try:
        ...
        print(f"\n?? IMPORTANT: Check {DEBUG_FILE} to see instrument structure")
        ...
        if data_written:
            ...
            print("\n" + "=" * 70)
            print("?? No data collected for any symbol. Excel file not created.")
            print(f"Debug:  {DEBUG_FILE}")
            print("=" * 70)
